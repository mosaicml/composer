...
__all__ = ['NLPTrainerHparams', 'load', 'FitHparams', 'EvalHparams', 'ExperimentHparams']
...
class NLPTrainerHparams(TrainerHparams):
    ...
    training_scheme: Optional[str] = hp.optional(doc="specify if trainer should pretrain, finetune, or all", default='all')
    output_type: Optional[List[str]] = hp.optional(doc="output GLUE, GLUE-Large, per-task metrics, or all", default=None)
    ...
    def validate(self):
        ...
        if self.training_scheme != 'pretrain' and self.training_scheme != 'finetune' and self.training_scheme != 'all':
            raise ValueError('training_scheme must be one of "pretrain," "finetune," or "all"')
            
        if self.training_scheme == 'pretrain' and self.output_type is not None:
            raise ValueError('output_type is only defined for fine-tuned models')

        if self.training_scheme == 'finetune' and self.load_path is None: 
            raise ValueError('load_path to checkpoint must be specified if finetuning a model')

        if self.training_scheme == 'finetune' and self.output_type is None:
...
class ExperimentHparams(hp.Hparams):
    ...
    Unlike the :class:`.NLPTrainerHparams`, this class allows for multiple configurations
    ...
    .. testsetup::
        ...
        from composer.trainer.trainer_hparams import ExperimentHparams, FitHparams, EvalHparams, NLPTrainerHparams

        trainer_hparams = NLPTrainerHparams(
    ...
    Args:
        trainer (NLPTrainerHparams): The trainer hparams.
    ...
    """
    trainer: NLPTrainerHparams = hp.required('NLP Trainer hparams')
