...
if version.parse(torch.__version__) > version.parse('2.2.9') and version.parse(
            ...
            if not fsdp_state._state_dict_config._use_dtensor:
                ...
                def _my_ext_all_gather_dtensor(
                 ...
                 fsdp_extension = None,
                    ...
                    print(f"bigning debug {fsdp_extension=}, {inspect.getsource(all_gather_dtensor_fn)}, before all_gather_shape: {tensor.shape}, local shape: {tensor._local_tensor.shape} after {t.shape}")
                    return t
    ...
    def redistribute_local_tensor(
        ...
        for i, (current, target) in sorted_placements:
            ...
            if current == target:
                ...
                continue
            print(f"bigning debug redistribute tensor {target.is_replicate()=}, {target.is_shard()=}, {current.is_shard()=}, {current.is_partial()=}")
    ...
    from torch.distributed._tensor import redistribute
