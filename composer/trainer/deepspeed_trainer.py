...
class DeepSpeedTrainer:
    ...
    def _train_loop(self) -> None:
        ...
        state.train_dataloader = self.dl_hparams.initialize_object(batch_size=state.train_batch_size / state.world_size,
        ...
        for _ in range(state.epoch, state.max_epochs):
            try:
                ...
                for batch_idx, state.batch in enumerate(state.train_dataloader):
                    print(state.batch)
                    state.batch = {k: v.to(device='cuda:0') for (k, v) in state.batch_dict.items()}
                    """state.batch = (
