...
class MNLImHparams(DatasetHparams):
    ...
    def validate(self):
        if self.split not in ['train', 'validation_mismatched', 'validation_matched', 'test']:
            raise ValueError("Split must be one of 'train', 'validation_{mismatched, matched}', 'test'.")
    ...
    def initialize_object(self) -> DataloaderSpec:
        ...
        self.dataset = datasets.load_dataset("glue", "mnli", split=self.split)
        print("Loading MNLI...")

        n_cpus = cpu_count()
        log.info(f"Starting tokenization step by preprocessing over {n_cpus} threads!")
        text_column_names = ["premise", "hypothesis"]
        ...
        self.dataset = self.dataset.map(
            ...
            new_fingerprint=f"mnli-tokenization-{self.split}",
