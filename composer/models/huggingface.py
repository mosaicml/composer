...
class HuggingFaceModel(ComposerModel):
    ...
    def eval_forward(self, batch, outputs: Optional[Any] = None):
        ...
        if batch.get('mode', None) == 'generate':
            ...
            if tokenizer_needs_prefix_space(self.tokenizer):
        ...
        if batch.get('mode', None) == 'mtbench':
            ...
            if tokenizer_needs_prefix_space(self.tokenizer):
                ...
                generation_one = self.tokenizer.batch_decode(first_generation[:, input_shape[1]:],
                                                             skip_special_tokens=True)
                generation_two = self.tokenizer.batch_decode(second_generation[:, batched_combined_prompts.shape[1]:],
                                                             skip_special_tokens=True)
            else:
                generation_one = [
                    ' ' + generation for generation in self.tokenizer.batch_decode(first_generation[:, input_shape[1]:],
                                                                                   skip_special_tokens=True)
                ]
                generation_two = [
                    ' ' + generation for generation in self.tokenizer.batch_decode(
                        second_generation[:, batched_combined_prompts.shape[1]:], skip_special_tokens=True)
                ]
