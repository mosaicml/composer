...
Profilers include:
* :class:`~composer.profiler.Profiler`:  Produces a trace of the training graph when the Composer Trainer is used.
...
* :class:`~composer.profiler.dataloader_profiler.DataloaderProfiler`: Records the time it takes the data loader to 
    return a batch by wrapping the original training and evaluation data loaders.  Implemented as a Composer :class:`Callback`.
* :class:`~composer.profiler.system_profiler.SystemProfiler`: Records system level metrics such as CPU, system memory,
    disk and network utilization.  Implemented as a Composer :class:`Callback`.
* :class:`~composer.profiler.torch_profiler.TorchProfiler`: Integrates the Torch Profiler to record GPU stats using the
    Nvidia CUPI API.  Implemented as a Composer :class:`Callback`.
"""
