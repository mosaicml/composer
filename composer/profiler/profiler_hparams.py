...
class MosaicProfilerHparams(hp.Hparams):
    hparams_registry = {
        "trace_event_handlers": {
            "json": JSONTraceHparams
        }
    }
    trace_event_handlers: List[ProfilerEventHandlerHparams] = hp.optional("Trace event handler hparams", default_factory=lambda: [JSONTraceHparams()])

    def initialize_object(self, state: State) -> MosaicProfiler:
        return MosaicProfiler(
            state, [x.initialize_object() for x in self.trace_event_handlers])
