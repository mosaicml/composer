...
from composer.utils.object_store import ObjectStore, OCIObjectStore, S3ObjectStore, LibcloudObjectStore
...
def maybe_create_object_store_from_uri(uri: str) -> Optional[ObjectStore]:
    ...
    if backend == 's3':
        ...
        if "GCS_KEY" not in os.environ or "GCS_SECRET" not in os.environ:
            raise ValueError("You must set the GCS_KEY and GCS_SECRET env variable with you HMAC access id and secret respectively")
            
        return LibcloudObjectStore(provider="google_storage",
...
def maybe_create_remote_uploader_downloader_from_uri(
    ...
    if backend in ['s3', 'oci']:
        ...
        if "GCS_KEY" not in os.environ or "GCS_SECRET" not in os.environ:
            raise ValueError("You must set the GCS_KEY and GCS_SECRET env variable with you HMAC access id and secret respectively")
        return RemoteUploaderDownloader(bucket_uri=f'libcloud://{bucket_name}',
                                        backend_kwargs={
