...
for PYTHON_VERSION in ${PYTHON_VERSIONS[@]}; do
    for PYTORCH_VERSION in ${PYTORCH_VERSIONS[@]}; do
        ...
        for CUDA_VERSION in ${CUDA_VERSIONS[@]}; do
            ...
            ARGS="TAG=mosaicml/pytorch:$PYTORCH_VERSION-${CUDA_VERSION}-python${PYTHON_VERSION}-${LINUX_DISTRO}"
            ARGS="$ARGS BASE_IMAGE='$BASE_IMAGE'"
            ARGS="$ARGS PYTHON_VERSION='$PYTHON_VERSION'"
            ARGS="$ARGS PYTORCH_VERSION='$PYTORCH_VERSION+$CUDA_VERSION'"
            ARGS="$ARGS TORCHVISION_VERSION='$TORCHVISION_VERSION+$CUDA_VERSION'"
            echo $ARGS
        done
    done
done
