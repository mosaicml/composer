...
class TestCheckpointResumption:
    ...
    @pytest.mark.filterwarnings('ignore:Please use DTensor instead*')
    @pytest.mark.filterwarnings('ignore:TypedStorage is deprecated*')
    @pytest.mark.filterwarnings('ignore:The ``compute`` method of metric*')
    # @pytest.mark.filterwarnings(
    #     'ignore:``FullyShardedDataParallel.scatter_full_optim_state_dict``is being deprecated and is replaced by.*')
    def test_fsdp_planner(
        ...
        self._assert_expected_num_checkpoints(
            save_folder=os.path.join(save_folder, 'first'),
            save_interval=save_interval,
            num_epochs=1,  # set in max_duration
            num_batches_per_epoch=2,  # set in train_subset_num_batches
            is_deepspeed=False,
        )

        resume_file = os.path.join(save_folder, 'first', resume_folder)
