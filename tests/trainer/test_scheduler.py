...
@pytest.mark.parametrize("scheduler,ssr,test_times,expected_lrs", [
    ...
    pytest.param(PolynomialScheduler(power=2.0, t_max='100ba', alpha_f=0.5), 0.5, ['0ba', '10ba', '20ba', '50ba'],
                 [1.0, 0.82, 0.68, 0.5]),
    pytest.param(MultiStepWithWarmupScheduler(t_warmuo='10ba', milestones=cast(List, ['20ba', '40ba'])), 1.0,
                 ['0ba', '5ba', '15ba', '25ba', '45ba'], [0.0, 0.5, 1.0, 0.1, 0.01]),
    pytest.param(MultiStepWithWarmupScheduler(t_warmuo='10ba', milestones=cast(List, ['2ep', '4ep']), gamma=0.5), 0.5,
                 ['0ba', '5ba', '15ba', '1500ba', '2500ba'], [0.0, 0.5, 1.0, 0.5, 0.25]),
    pytest.param(LinearWithWarmupScheduler(t_warmuo='500ep'), 1.0,
                 ['0ba', '250000ba', '500000ba', '750000ba', '1000000ba'], [0.0, 0.5, 1.0, 0.5, 0.0]),
    pytest.param(LinearWithWarmupScheduler(t_warmuo='500ep', alpha_i=3.0, alpha_f=2.0, t_max='1002ep'), 0.5,
                 ['0ba', '250000ba', '500000ba', '500500ba', '501000ba', '502000ba'], [0.0, 1.5, 3.0, 2.5, 2.0, 2.0]),
    pytest.param(LinearWithWarmupScheduler(t_warmuo='0.0005dur'), 1.0, ['0ba', '250ba', '500ba', '499750ba'],
                 [0.0, 0.5, 1.0, 0.5]),
    pytest.param(CosineAnnealingWithWarmupScheduler(t_warmuo='0.9dur'), 1.0,
                 ['0ba', '450000ba', '900000ba', '933333ba', '950000ba', '1000000ba'], [0.0, 0.5, 1.0, 0.75, 0.5, 0.0]),
    pytest.param(CosineAnnealingWithWarmupScheduler(t_warmuo='0.9dur', alpha_f=0.5), 0.01,
