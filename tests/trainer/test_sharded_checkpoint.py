...
class SimpleMLP(ComposerClassifier):
    ...
    def __init__(self, num_features: int, num_classes: int = 4):
        net = torch.nn.Sequential(torch.nn.Linear(num_features, num_features, bias=True),
                                       torch.nn.ReLU(),
...
#                     reason='TODO: figure out if this test should work with torch2')
...
#     ShardedTensor = pytest.importorskip('torch.distributed.fsdp.fully_sharded_data_parallel.ShardedTensor')
#     save_folder = tmp_path
#     save_filename = 'rank{rank}.pt'
