...
def get_trainer(save_folder = None, save_filename = None, num_features=2, fsdp_state_dict_type='full', load_path=None):
...
def _compare_optims_between_state_dicts(state_dict1, state_dict2):
    ...
    for param_name in state_dict2_optim_params.keys():
        ...
        for moment_name in state_dict2_param_moment_dict.keys():
            ...
            assert torch.equal(state_dict1_moment, state_dict2_moment), f"Moment {moment_name} for parameter {param_name} not the same between state dicts"
...
def _compare_model_params_between_state_dicts(state_dict1, state_dict2):
    ...
    for param_name in state_dict2_model_params.keys():
        ...
        assert torch.equal(state_dict1_model_tensor, state_dict2_model_tensor), f"Weight named {param_name} not the same between state_dicts"
...
def test_fsdp_full_state_dict_save(world_size, tmp_path: pathlib.Path):
    ...
    if dist.get_global_rank() == 0:
        ...
        _compare_model_params_between_state_dicts(state_dict_from_checkpoint,
                                                 state_dict_in_memory)

        _compare_optims_between_state_dicts(state_dict_from_checkpoint,
                                                 state_dict_in_memory)
...
def test_fsdp_full_state_dict_load(world_size, tmp_path: pathlib.Path):
    ...
    if dist.get_global_rank() == 0:
        _compare_model_params_between_state_dicts(state_dict_from_trainer1,
                                                 state_dict_from_trainer2)

        _compare_optims_between_state_dicts(state_dict_from_trainer1,
                                                 state_dict_from_trainer2)
