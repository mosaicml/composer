...
def get_trainer(
        ...
        fsdp_config: TestFSDPConfig = TestFSDPConfig(),
    ):
    ...
    trainer = Trainer(
        ...
        fsdp_config=fsdp_config.as_dict(),
...
def test_fsdp_mixed_with_sync(
    ...
    if not sync_module_states:
        pytest.raises(ValueError, match='Detected mixed initialization.*')

    with contextlib.nullcontext():
        get_trainer(
            model_init_device=['cpu', 'meta'][dist.get_global_rank()],
            save_folder=str(tmp_path),
            fsdp_config = TestFSDPConfig(sync_module_states=sync_module_states),
        )
    ...
    pytest.param(
        ...
        marks=[
            pytest.mark.filterwarnings(
                (r'ignore:ShardedGradScaler is not in the state_dict. Its state'
...
def test_fsdp_load_old_checkpoint(
    ...
    if (version.parse(torch.__version__) >= version.parse('1.13.0')
        and composer_version not in ['0.13.5', '0.14.0', '0.14.1']):
        pytest.skip(
            ('Composer 0.15.1 and above checkpoints were saved with torch 2 and'
             ' as a result are not compatible with torch 1.13.')
        )
    if (version.parse(torch.__version__) >= version.parse('2.0.0')
        and state_dict_type == 'local'):
        pytest.xfail(
            ('Loading a torch 1.13 checkpoint with torch 2.0 for '
    ...
    if composer_version in ['0.13.5', '0.14.0', '0.14.1', '0.15.1']:
        ...
        if ((version.parse(composer_version) > version.parse('0.15.0'))
            and state_dict_type != 'full'):
    ...
    if ((dist.get_global_rank() == 0 and state_dict_type == 'full')
        or state_dict_type in ['sharded', 'local']):
...
def test_fsdp_full_state_dict_load_with_ema(
    ...
    fsdp_config = TestFSDPConfig(sharding_strategy='SHARD_GRAD_OP')
...
def test_fsdp_partitioned_state_dict_load(
    ...
    fsdp_config = TestFSDPConfig(state_dict_type=state_dict_type)
...
@pytest.mark.filterwarnings(r'ignore:TypedStorage is deprecated.:UserWarning')
@pytest.mark.filterwarnings(
    r'ignore:MosaicMLLogger is not in the state_dict.:UserWarning'
)
@pytest.mark.filterwarnings(
    r'ignore:.*metrics are not saved with sharded state dict.*:UserWarning'
)
def test_elastic_resumption(
    ...
    def compare_state_dicts():
        ...
        state_dict_from_trainer2 = get_mono_state_dict_from_sharded_one(
            sharded_trainer
        )
        _compare_model_params_between_state_dicts(
            state_dict_from_trainer1,
...
def test_cleanup_sharded_checkpoints(
    ...
    dir_contents = [
        file_or_dir for file_or_dir in os.listdir(shards_dir)
        if 'latest' not in file_or_dir
    ]
