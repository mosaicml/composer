...
def test_time_convert_to_duration():
    conversion = Time("200ep").convert("dur", max_training_duration="400ep")
    assert conversion == 0.5
    assert conversion.unit == "dur"


def test_time_convert_from_duration():
    conversion = Time("0.5dur").convert("ep", max_training_duration="400ep")
    assert conversion == 200
    assert conversion.unit == "ep"
