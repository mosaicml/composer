...
@pytest.mark.parametrize('interpolation', [0, 0.1, 0.5, 0.9, 1.0])
@pytest.mark.parametrize('tensors', generate_tensors())
class TestLabelSmoothing:

    @staticmethod
    def reference_smooth_labels(targets, interpolation):
        num_classes = targets.shape[-1]
        return targets * (1 - interpolation) + interpolation / num_classes

    def test_label_smoothing(self, tensors, interpolation):
        (input, target_indices, target_onehot) = tensors

        labels_onehot = label_smoothing.smooth_labels(input, target_onehot, interpolation)
        labels_indices = label_smoothing.smooth_labels(input, target_indices, interpolation)
        labels_ref = self.reference_smooth_labels(target_onehot, interpolation)
    ...
    def test_label_smoothing_algorithm(self, tensors, interpolation, target_type, empty_logger, minimal_state):
        (outputs, target_indices, target_onehot) = tensors

        target = target_indices if target_type == 'indices' else target_onehot

        algorithm = LabelSmoothingHparams(interpolation=interpolation).initialize_object()
        state = minimal_state
        ...
        smoothed_reference = self.reference_smooth_labels(target_onehot, interpolation)
...
def test_label_smoothing_match():
    algorithm = LabelSmoothingHparams(interpolation=0.1).initialize_object()
