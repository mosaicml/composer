...
@pytest.mark.timeout(10)
def test_progress_bar_dataloader_label_dict(max_duration: Time[int], monkeypatch: MonkeyPatch, world_size: int):
    model = SimpleModel()
    ...
    trainer = Trainer(model=model,
                      ...
                      eval_subset_num_batches=eval_subset_num_batches,
                      loggers=[
                          ProgressBarLogger(True,
                                            dataloader_label_to_metrics={
                                                'eval2': False,
                                                'eval1': 'accuracy={Accuracy}, custom message'
                                            })
                      ])
