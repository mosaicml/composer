...
def test_eval_output_logging(device, subset_samples, tmp_path, tiny_gpt2_tokenizer):
    ...
    for i in range(2):
        ...
        assert all(len(m.response_cache) == 0 for dictionary in state.eval_metrics.values() for m in dictionary.values()) # pyright: ignore[reportGeneralTypeIssues]
        ...
        assert  json.loads(in_memory_logger.tables['icl_outputs/lm_acc/InContextLearningLMAccuracy()'])['columns'] == ["context_tok","continuation_tok_target","continuation_tok_pred","correct"] 
        if subset_samples == -1:
            assert  json.loads(in_memory_logger.tables['icl_outputs/lm_acc/InContextLearningLMAccuracy()'])['data'] == [['The dog is', ' furry', ' furry', True], ['I love to eat', ' pie', '', False], ['I hate', ' long lines', ' long lines', True], ['The weather is', ' snowy', ' snowy', True]]
        else:
            assert len(json.loads(in_memory_logger.tables['icl_outputs/lm_acc/InContextLearningLMAccuracy()'])['data']) == subset_samples
        ...
        eval_output_logging.eval_start(state, logger)
        assert all(len(m.response_cache) == 0 for dictionary in state.eval_metrics.values() for m in dictionary.values()) # pyright: ignore[reportGeneralTypeIssues]
        mock_mc_computation(state.eval_metrics['mc_acc']['InContextLearningMultipleChoiceAccuracy()'], tiny_gpt2_tokenizer)
        ...
        assert  json.loads(in_memory_logger.tables['icl_outputs/mc_acc/InContextLearningMultipleChoiceAccuracy()'])['columns'] == ["question_tok","correct_choice","selected_choice","correct"]
        if subset_samples == -1:
            assert  json.loads(in_memory_logger.tables['icl_outputs/mc_acc/InContextLearningMultipleChoiceAccuracy()'])['data'] == [["Q: How do you cook a cake?"," A: turn on the oven"," A: turn on the oven",True],["Q: How old is the earth?"," A: 4.5 billion years"," A: 2 minutes",False]]
        else:
            assert len(json.loads(in_memory_logger.tables['icl_outputs/mc_acc/InContextLearningMultipleChoiceAccuracy()'])['data']) == subset_samples
        ...
        assert set(df['benchmark']) == {'mc_acc_InContextLearningMultipleChoiceAccuracy()', 'lm_acc_InContextLearningLMAccuracy()'}
        assert df.columns.tolist() == ['context_tok', 'continuation_tok_target', 'continuation_tok_pred', 'correct', 'benchmark', 'question_tok', 'correct_choice', 'selected_choice']
