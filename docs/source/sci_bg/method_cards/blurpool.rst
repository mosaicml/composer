...
Blurpool was proposed in `Making Convolutional Networks Shift-Invariant Again
<http://proceedings.mlr.press/v97/zhang19a/zhang19a.pdf>`__ by Richard Zhang, and 
subsequently reproduced in many papers <CITE>

- Proposes to do a 2D anti-aliasing filter between computing the max and pooling; also propose doing this for strided convs and local average pooling
- One downside of this approach is that you do 4x as much work; they compute largest entry in 2x2 window for each input px, and then downsample, rather than just compute max in 2x2 window for each output pixel
- Similarly, for strided convolutions, you just use a stride of 1 and then blur + downsample, instead of using a stride of 2
