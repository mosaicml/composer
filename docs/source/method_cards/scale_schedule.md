...
Scale schedule supports all Composer Schedulers. See the Scheduling Guide for more information on how Composer Schedulers work.

Scale schedule also supports the following PyTorch Schedulers:
* {class}`~torch.optim.StepLR`
* {class}`~torch.optim.MultiStepLR`
* {class}`~torch.optim.ExponentialLR`
* {class}`~torch.optim.CosineAnnealingLR`
* {class}`~torch.optimCosineAnnealingWarmRestarts`.
...
For additional details on using the scale schedule ratio, see the scheduling guide TODO LINK.
